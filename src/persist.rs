...
impl IvfIndex {
    pub fn save_dir(&self, dir: &Path) -> io::Result<()> {
        ...
        // lists v2: tomb | ids | vecs | has_tags? | tags
        ...
        for list in &self.lists {
            ...
            if has_tags {
                for &mask in list.tags.as_ref().unwrap() { w.write_all(&mask.to_le_bytes())?; }
            }
        }
    ...
    pub fn load_dir(dir: &Path) -> io::Result<IvfIndex> {
        ...
        while off < cent_bytes.len() {
            ...
            for d in 0..dim {
                let mut b = [0u8;4]; b.copy_from_slice(&cent_bytes[off..off+4]); off += 4;
                row[d] = f32::from_le_bytes(b);
            }
            centroids.push(row);
        }
        ...
        for _ in 0..nlist {
            ...
            for _ in 0..cnt {
                let mut b=[0u8;8]; b.copy_from_slice(&lb[p..p+8]); p+=8;
                ids.push(u64::from_le_bytes(b));
            }
            let mut vecs = Vec::with_capacity(cnt*dim);
            for _ in 0..cnt*dim {
                let mut b=[0u8;4]; b.copy_from_slice(&lb[p..p+4]); p+=4;
                vecs.push(f32::from_le_bytes(b));
            }
        ...
        let header = man.header;     // move out once
        let params = header.params;  // IvfParams: Copy

...
fn run_ivf_flat(
    ...
    let perq: Vec<PerQ> = parallel_map_indexed(eval_slice, threads, |q, _| {
        let t = Instant::now();
        let ann1 = ivf.search(q, k);
        let dt = t.elapsed().as_micros() as u128;
        let det_ok = ann1 == ivf.search(q, k);
        ...
        let mut hits = 0usize;
        for id in truth_ids.iter().take(k) { if ann_ids.contains(id) { hits += 1; } }
        PerQ { hits, poss: k, dt_us: dt, det_ok }
    });
...
fn run_ivf_pq(
    metric: Metric,
    dim: usize,
    data: &[(u64, Vec<f32>, u64)], // UPDATED to 3-tuple
    flat: &FlatIndex,
    queries: &[Vec<f32>],
    warmup: usize,
    k: usize,
    ...
    let perq: Vec<PerQ> = parallel_map_indexed(eval_slice, threads, |q, _| {
        ...
        let dt = t.elapsed().as_micros() as u128;
        let det_ok = ann1 == ivfpq.search(q, k);
...
fn main() {
    ...
    // UPDATED: Standardizing data for both backends (IVF-PQ needs timestamps)
    let mut data_raw: Vec<(u64, Vec<f32>)> = Vec::with_capacity(args.n);
    let mut data_temporal: Vec<(u64, Vec<f32>, u64)> = Vec::with_capacity(args.n);
    ...
    for i in 0..args.n {
        ...
        flat.add(i as u64, &v);
        data_raw.push((i as u64, v.clone()));
        data_temporal.push((i as u64, v, i as u64)); // i as synthetic timestamp
    }
        ...
        for &refine in &refine_list {
            ...
            let (row, _sum_us, det) = match args.backend {
                Backend::IvfFlat => run_ivf_flat(
                    metric, args.dim, &data_raw, &flat, &queries, args.warmup, args.k,
                    args.nlist, nprobe, refine, args.seed_kmeans, args.threads,
                ),
                Backend::IvfPq => run_ivf_pq(
                    metric, args.dim, &data_temporal, &flat, &queries, args.warmup, args.k,
                    args.nlist, nprobe, refine, args.seed_kmeans,
                    args.m, args.nbits, args.iters, args.opq, args.opq_mode.into(),
                    args.opq_sweeps, args.store_vecs, args.threads,
                ),
            };
            ...
            if let Some(w) = csv.as_mut() {
                writeln!(
                    ...
                    row.recall, row.lb, (row.p50_ms * 1000.0).round(), (row.p90_ms * 1000.0).round(),
                    (row.p95_ms * 1000.0).round(), (row.p99_ms * 1000.0).round(), row.qps, row.build_det,
                    dataset_ms, eval_ms, if matches!(args.backend, Backend::IvfPq) { args.m as i64 } else { -1 },
                    ...
                    if matches!(args.backend, Backend::IvfPq) { args.store_vecs } else { false }, args.threads,
                ).ok();
            }
            rows.push(row);
        }
    ...
    if args.enforce {
        if args.require_all {
            if rows.iter().any(|r| r.lb < args.target_lb) { std::process::exit(1); }
        } else {
            rows.sort_by(|a, b| b.lb.partial_cmp(&a.lb).unwrap());
            if rows[0].lb < args.target_lb { std::process::exit(1); }
        }
    }
}